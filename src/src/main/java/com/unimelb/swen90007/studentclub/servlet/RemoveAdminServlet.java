    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection connection = DatabaseConnection.getConnection()) {
            ...
            if (!adminDAO.isAdmin(loggedInStudent.getId(), clubId, unitOfWork)) {
                ...
                response.sendError(HttpServletResponse.SC_FORBIDDEN, "You are not authorized to remove admins for this club.");
                return;
            }

            // Register the operation to remove the admin in the UnitOfWork
            adminDAO.removeAdmin(studentIdToRemove, clubId, unitOfWork);
            ...
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "An error occurred while removing the admin.");
        }
    }
}
